...
async def _bridge(request: Request, operation: str, target_path: str, permission: str, purpose: str, body: dict[str, Any], path_params: dict[str, str] | None = None, method: str = "POST"):
    ...
    try:
        delegation = issue_fiteatsy_delegation(
            subject=str(getattr(request.state, "user_id", None) or payload.get("sub")),
    ...
    url = f"{settings.fiteatsy_service_url.rstrip('/')}{path}"
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "X-Zestiva-Delegation": delegation,
