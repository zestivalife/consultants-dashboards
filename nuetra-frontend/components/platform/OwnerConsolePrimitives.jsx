...
export function StatPill({ icon: Icon, label, value, tone = 'from-[#237afc] to-[#58b6ff]' }) {
  return (
    <div className="rounded-2xl border border-gray-100/80 bg-gradient-to-br from-white to-gray-50/80 p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="flex items-center justify-between">
        <div>
          <p className="z-label tracking-[0.14em] text-gray-400">{label}</p>
          <p className="mt-3 text-2xl font-semibold tracking-tight text-gray-900">{value}</p>
        </div>
        <div className={cn('rounded-2xl bg-gradient-to-br p-3 text-white shadow-[0_8px_18px_rgba(15,23,42,0.08)]', tone)}>
      ...
      <div className="rounded-3xl border border-gray-100/80 bg-white p-6 shadow-[0_1px_3px_rgba(15,23,42,0.05)] md:p-7">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl z-body text-gray-500">{description}</p>
          </div>
...
export function ActionButton({ icon: Icon, label, tone = 'secondary', onClick, type = 'button', disabled = false }) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'inline-flex items-center gap-2 rounded-xl px-4 py-3 z-btn transition-colors disabled:cursor-not-allowed disabled:opacity-60',
...
export function ControlBar({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-gray-100/80 bg-gray-50/70 p-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {filters.map((filter) => (
          <button
            ...
            className={cn(
              'rounded-full border px-4 py-2 z-btn transition-colors',
...
export function Badge({ children, tone = 'neutral' }) {
  ...
  return <span className={cn('rounded-full border px-3 py-1 z-label', tones[tone])}>{children}</span>;
}

export function Panel({ title, subtitle, action, children }) {
  return (
    <div className="rounded-3xl border border-gray-100/80 bg-white p-6 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="z-h3 text-gray-900">{title}</h3>
          {subtitle ? <p className="mt-1 z-body text-gray-500">{subtitle}</p> : null}
        </div>
        {action}
      </div>
...
export function EmptyState({ icon: Icon, title, description, action }) {
  return (
    <div className="rounded-3xl border border-dashed border-gray-200 bg-white px-6 py-16 text-center">
      ...
      <p className="mt-2 z-body text-gray-500">{description}</p>
...
export function WorkflowFooter({ children }) {
  return (
    <footer className="sticky bottom-0 z-20 shrink-0 border-t border-gray-100 bg-white px-5 py-3.5 md:px-6">
      {children}
    </footer>
  );
}
    ...
    <div className="-mx-1 flex min-h-[60px] gap-2 overflow-x-auto px-1 pb-1" aria-label="Workflow steps">
      {steps.map((step, index) => (
        ...
        >
          ...
          <span className="mt-1 block z-table-content font-medium">{step.label}</span>
        </button>
      ))}
    </div>
  );
...
export function ReviewCard({ label, value }) {
  return (
    <div className="rounded-2xl bg-gray-50 px-4 py-3">
      ...
      <p className="mt-1 z-table-content font-medium text-gray-900">{value}</p>
    </div>
  );
}
