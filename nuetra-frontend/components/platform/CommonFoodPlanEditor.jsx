...
const CommonFoodPlanEditor = forwardRef(function CommonFoodPlanEditor({ clientId, dietPlanId, planVersionId, lifecycle, initialOptions = [], legacyMealPlan = {}, generationRequestId = 0, readOnly = false, onStale, onDirtyChange, onProgressChange }, ref) {
  ...
  const selectFood = async (component) => {
    ...
    if (explorer.mode === 'create') {
      setError('');
      try {
        const created = await saveFiteatsyCommonFoodOption(clientId, dietPlanId, { expectedPlanVersionId: planVersionId, mealHead: explorer.mealHead, components: [foodComponent] });
        const next = { ...created, sourceType: explorer.authoringSource || 'MANUAL' };
        setOptions((current) => [...current.filter((item) => item.combinationId !== next.combinationId), next]);
        setSelectedIds((current) => new Set([...current, next.combinationId]));
        setDirty(true); setExplorer(null); setMessage('New meal option added. Continue composing it if needed, then save the complete Diet Plan.');
      } catch (nextError) { if (nextError?.status === 409) onStale?.(); setError(commonFoodErrorMessage(nextError)); }
      return;
    }
  ...
  const reusePreviousOption = async (option) => {
    const reuse = previousOptionReuseState(option);
    if (!reuse.reusable) { setError(reuse.reason); return; }
    setLoading(true); setError('');
    try {
      const created = await saveFiteatsyCommonFoodOption(clientId, dietPlanId, { expectedPlanVersionId: planVersionId, mealHead: option.mealHead, components: option.components.map(({ foodId, servingId, multiplier }) => ({ foodId, servingId, multiplier: Number(multiplier) })) });
      const next = { ...created, sourceType: 'PREVIOUS_PLAN' };
      setOptions((current) => [...current.filter((item) => item.combinationId !== next.combinationId), next]);
      setSelectedIds((current) => new Set([...current, next.combinationId]));
      setDirty(true); setMessage('Previous-plan option revalidated against current hard constraints and included. Save to persist it.');
    } catch (nextError) { if (nextError?.status === 409) onStale?.(); setError(commonFoodErrorMessage(nextError, 'This previous option is not currently reusable.')); }
    finally { setLoading(false); }
  };

  const hasOptions = unifiedOptions.length > 0 || meals.length > 0 || (!readOnly && ['draft', 'changes_requested'].includes(lifecycle));
  ...
  return <section aria-label="Diet Plan" className="space-y-4">
    ...
    {!hasOptions ? <div className="rounded-[18px] border border-dashed bg-[var(--fluent-color-neutral-background-2)] p-8 text-center text-sm text-[var(--fluent-color-neutral-foreground-2)]">{loading ? 'Generating personalised options for all seven meals…' : 'No generated options are available yet. Add an approved food, build a meal, or generate alternatives.'}</div> : <div className="grid items-start gap-4 xl:grid-cols-[220px_minmax(0,1fr)]"><nav className="sticky top-[198px] flex gap-2 overflow-x-auto rounded-[16px] bg-[var(--fluent-color-neutral-background-2)] p-2 xl:block xl:space-y-2" aria-label="Meal navigator">{COMMON_FOOD_MEALS.map(([head,label])=>{const count=(byMeal[head]||[]).filter((option)=>selectedIds.has(option.combinationId)).length;const available=buildMealAuthoringPool(byMeal[head]||[],legacyByMeal[head]||[]).length;return <button key={head} type="button" onClick={()=>setActiveMealHead(head)} className={`flex min-w-[150px] items-center justify-between gap-3 rounded-[12px] px-3 py-2.5 text-left text-xs xl:w-full ${activeMealHead===head?'bg-[var(--fluent-color-brand-background)] text-[var(--fluent-color-brand-foreground)]':'bg-[var(--fluent-color-neutral-background-1)] text-[var(--fluent-color-neutral-foreground-2)]'}`}><span className="font-semibold">{label}</span><span className="shrink-0 font-semibold">{count}/5{count===5?' ✓':''} · {available} available</span></button>})}</nav><div className="space-y-3">{COMMON_FOOD_MEALS.filter(([head])=>head===activeMealHead).map(([head, label]) => {
      const generatedMeal = meals.find((item) => item.mealHead === head);
      const mealOptions = byMeal[head] || [];
      const previousMealOptions = legacyByMeal[head] || [];
      const authoringPool = buildMealAuthoringPool(mealOptions, previousMealOptions);
      const reusablePrevious = authoringPool.filter((option) => option.authoringSource === 'PREVIOUS_PLAN');
      ...
      return <div key={head} className="rounded-[18px] border bg-white p-4"><div className="flex flex-wrap justify-between gap-3"><div><h5 className="text-lg font-semibold">{label}</h5><p className="mt-1 text-xs text-gray-600">{authoringPool.length} available · {selectedCount} included · {persistedByMeal[head]} persisted</p><p className="mt-1 text-xs text-gray-600">Target {formatNutrient(mealTarget, 'kcal')}{targetStatus ? ` · Acceptable range ${formatNutrient(targetStatus.minimum, 'kcal')}–${formatNutrient(targetStatus.maximum, 'kcal')}` : ''}</p></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${selectedCount === 5 ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-800'}`}>{selectedCount}/5 included</span>{!readOnly ? <><button type="button" onClick={()=>setMealSelection(head,true)} className="text-xs font-semibold text-blue-700">Select all</button><button type="button" onClick={()=>setMealSelection(head,false)} className="text-xs font-semibold text-gray-600">Clear</button><button type="button" disabled={selectedCount >= 5} onClick={()=>openExplorer('create',null,null,label,'',{mealHead:head,authoringSource:'MANUAL'})} className="rounded-full border px-3 py-1.5 text-xs font-semibold disabled:opacity-40">+ Add Food</button><button type="button" disabled={selectedCount >= 5} onClick={()=>{if(mealOptions.length){setBuildMealHead(buildMealHead===head?'':head);setBuildOptionIds(new Set(mealOptions.filter((option)=>selectedIds.has(option.combinationId)).map((option)=>option.combinationId)))}else openExplorer('create',null,null,label,'STARCH',{mealHead:head,authoringSource:'BUILD_MEAL'})}} className="rounded-full border px-3 py-1.5 text-xs font-semibold disabled:opacity-40">+ Build Meal</button><button type="button" onClick={()=>void generate()} disabled={loading} className="rounded-full border px-3 py-1.5 text-xs font-semibold disabled:opacity-40">Generate alternatives</button></> : null}</div></div>
        ...
        <div className="mt-3 space-y-2">{mealOptions.map((option, index) => { const key = option.combinationId || `${head}-${index}`; const selected = selectedIds.has(option.combinationId); const state = persistedIds.has(option.combinationId) ? 'SAVED' : selected ? 'SELECTED' : 'AVAILABLE'; const calorie=mealCalorieStatus(option.nutrition?.kcal,mealTarget); const calorieLabel=calorie?.state==='WITHIN_TARGET'?'Within target':calorie?.state==='BELOW_TARGET'?'Below target':'Above target'; return <article key={key} data-option-type={option.sourceType} data-selection-state={state} className={`rounded-[12px] border px-3 py-2.5 ${selected ? 'border-[var(--fluent-color-brand-stroke-1)] bg-[var(--fluent-color-brand-background-2)]' : 'bg-[var(--fluent-color-neutral-background-2)]'}`}><div className="flex items-start gap-3">{!readOnly && ['draft', 'changes_requested'].includes(lifecycle) ? <input type="checkbox" checked={selected} onChange={() => toggleSelection(option)} aria-label={`${selected ? 'Exclude' : 'Include'} ${optionTitle(option)}`} className="mt-1 h-4 w-4 accent-[var(--fluent-color-brand-background)]" /> : null}<div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><p className="truncate text-sm font-semibold">Option {index+1} · {optionTitle(option)}</p><div className="flex items-center gap-2"><span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700">{option.sourceType === 'PREVIOUS_PLAN' ? 'Previous Plan' : option.sourceType === 'MANUAL' ? 'Manual' : option.sourceType === 'BUILD_MEAL' ? 'Build Meal' : 'Generated'}</span><span className={`text-[11px] font-semibold ${calorie?.state==='WITHIN_TARGET'?'text-green-700':'text-amber-800'}`}>{calorie ? `${calorie.variancePercent>=0?'+':''}${calorie.variancePercent.toFixed(1)}% · ${calorieLabel}${calorie.state==='WITHIN_TARGET'?' ✓':''}` : 'Calorie target unavailable'}</span></div></div><p className="mt-0.5 text-xs text-gray-600">{servingSummary(option)}</p><p className="mt-1 text-xs font-medium text-gray-700">{compactNutrition(option.nutrition)}</p><p className="mt-1 text-[11px] text-gray-500">Target {formatNutrient(mealTarget,'kcal')}{calorie ? ` · Current ${formatNutrient(calorie.current,'kcal')} · ${calorie.remaining>=0?'Remaining':'Over'} ${formatNutrient(Math.abs(calorie.remaining),'kcal')}` : ''}</p><div className="mt-2 flex flex-wrap gap-3 text-xs font-semibold">{!readOnly?<><button type="button" onClick={()=>setExpanded((current)=>({...current,[key]:true}))} className="text-blue-700">Edit</button><button type="button" onClick={()=>openExplorer('replace',option,option.components?.[0],label)} className="text-blue-700">Replace</button><button type="button" onClick={()=>{if(selected)toggleSelection(option)}} disabled={!selected} className="text-red-700 disabled:opacity-40">Remove</button></>:null}<button type="button" aria-expanded={Boolean(expanded[key])} onClick={()=>setExpanded((current)=>({...current,[key]:!current[key]}))} className="text-blue-700">{expanded[key]?'Hide composition':'Details'}</button></div>{expanded[key]?<><ComponentTable option={option} readOnly={readOnly || !['draft','changes_requested'].includes(lifecycle)} locked={lockedServings[option.combinationId]} onLock={(foodId)=>toggleServingLock(option.combinationId,foodId)} onExplore={(mode,component)=>openExplorer(mode,option,component,label)} onRemove={(component)=>mutate(()=>removeFiteatsyCommonFoodComponent(clientId,dietPlanId,option.combinationId,component.foodId,planVersionId))} onServing={(component)=>openExplorer('serving',option,component,label)}/>{!readOnly?<button type="button" onClick={()=>autoBalance(option)} className="mt-2 rounded-full border px-3 py-1.5 text-xs font-semibold">Balance to target</button>:null}</>:null}</div></div></article>; })}{Array.from({length:Math.max(0,5-mealOptions.length)},(_,slot)=><article key={`empty-${head}-${slot}`} data-slot-state="EMPTY" className="rounded-[12px] border border-dashed px-3 py-3"><p className="text-sm font-semibold">Option {mealOptions.length+slot+1} · Empty slot</p><p className="mt-1 text-xs text-gray-500">Fill with a reusable previous option, an approved food, or Build Meal.</p></article>)}{mealOptions.length < 5 ? <div className="rounded-[12px] border border-dashed p-4 text-sm text-gray-600"><p>{loading ? 'Generating personalised options…' : `No new generated options fill the remaining ${5 - Math.min(5, mealOptions.length)} slot${mealOptions.length === 4 ? '' : 's'} for ${label}.`}</p><p className="mt-1 text-xs">Reuse a current-safe previous option, add an approved food, or build a meal. Generator ranking is not a manual-authoring block.</p></div> : null}</div>
        {reusablePrevious.length ? <section className="mt-4 rounded-[14px] border border-dashed p-3" aria-label={`Reusable previous ${label} options`}><p className="text-xs font-semibold">Previous plan options available now · {reusablePrevious.length}</p><div className="mt-3 space-y-2">{reusablePrevious.map((option) => <article key={option.combinationId} data-option-type="PREVIOUS_PLAN" className="rounded-[12px] bg-[var(--fluent-color-neutral-background-2)] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">{option.displayName}</p><p className="mt-1 text-xs text-gray-600">{option.servingLabel} · {compactNutrition(option.nutrition)}</p></div>{!readOnly?<button type="button" disabled={loading || selectedCount >= 5} onClick={()=>void reusePreviousOption(option)} className="rounded-full border px-3 py-1.5 text-xs font-semibold disabled:opacity-40">Reuse</button>:null}</div><LegacyOptionDetails option={option} /></article>)}</div></section> : null}
        {previousMealOptions.filter((option)=>!previousOptionReuseState(option).reusable).length ? <details className="mt-3 rounded-[14px] border border-dashed p-3"><summary className="cursor-pointer text-xs font-semibold">Unavailable historical choices · {previousMealOptions.filter((option)=>!previousOptionReuseState(option).reusable).length}</summary><div className="mt-2 space-y-2">{previousMealOptions.filter((option)=>!previousOptionReuseState(option).reusable).map((option)=><p key={option.combinationId} className="text-xs text-gray-600">{option.displayName}: {previousOptionReuseState(option).reason}</p>)}</div></details> : null}
      </div>;
    })}</div></div>}
