      ...
      <tbody className="divide-y divide-[var(--fluent-color-neutral-stroke-1)]">
        {(option.components || []).map((component) => <tr key={component.componentId || component.foodId}>
          ...
          <td className="py-2"><div className="flex gap-2">
            {!readOnly ? <><button type="button" onClick={() => onExplore('replace', component)} className="font-semibold text-[var(--fluent-color-brand-foreground-link)]">Change</button><button type="button" onClick={() => onServing(component)} className="font-semibold text-[var(--fluent-color-brand-foreground-link)]">Adjust portion</button><button type="button" aria-pressed={locked?.has(component.foodId)} onClick={() => onLock(component.foodId)} className="font-semibold text-[var(--fluent-color-brand-foreground-link)]">{locked?.has(component.foodId) ? 'Unlock' : 'Lock serving'}</button><button type="button" onClick={() => onRemove(component)} className="font-semibold text-[var(--fluent-color-status-danger-foreground)]">Remove</button></> : <span>Snapshot</span>}
          </div></td>
        </tr>)}
      </tbody>
    </table>
...
const optionTypeLabels = { LEGACY: 'Legacy option', VALIDATED_RECIPE: 'Validated recipe', GENERATED_COMBINATION: 'Generated combination' };
...
const CommonFoodPlanEditor = forwardRef(function CommonFoodPlanEditor({ clientId, dietPlanId, planVersionId, lifecycle, initialOptions = [], legacyMealPlan = {}, generationRequestId = 0, readOnly = false, onStale, onDirtyChange }, ref) {
  ...
  return <section aria-label="Diet Plan" className="space-y-4">
    <div><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-base font-semibold">Diet Plan Progress — {selectedTotal} / 35 selected</h4><p className="mt-1 text-sm text-[var(--fluent-color-neutral-foreground-2)]">{remainingTotal ? `${remainingTotal} selections remaining. Available candidates and saved selections are tracked separately.` : 'All required selections are complete.'}</p></div><button type="button" onClick={() => window.print()} className="rounded-full border px-3 py-2 text-xs font-semibold">Preview as Client</button></div>{dirty ? <p className="mt-3 text-xs font-semibold text-amber-700">Unsaved Diet Plan changes</p> : null}{error ? <p role="alert" aria-live="assertive" className="mt-3 rounded-[12px] bg-red-50 p-3 text-sm text-red-700">{error}{error.includes('newer plan version') ? <button type="button" onClick={() => reload()} className="ml-2 underline">Reload latest</button> : null}</p> : null}{message ? <p role="status" className="mt-3 rounded-[12px] bg-green-50 p-3 text-sm text-green-700">{message}</p> : null}</div>
    {!hasOptions ? <div className="rounded-[18px] border border-dashed bg-[var(--fluent-color-neutral-background-2)] p-8 text-center text-sm text-[var(--fluent-color-neutral-foreground-2)]">{loading ? 'Generating personalised options for all seven meals…' : 'Generate a Diet Plan to create five personalised options for each meal.'}</div> : <div className="grid items-start gap-4 xl:grid-cols-[220px_minmax(0,1fr)]"><nav className="space-y-2 rounded-[16px] bg-[var(--fluent-color-neutral-background-2)] p-2" aria-label="Meal navigator">{COMMON_FOOD_MEALS.map(([head,label],index)=>{const count=(byMeal[head]||[]).filter((option)=>selectedIds.has(option.combinationId)).length;return <button key={head} type="button" onClick={()=>setActiveMealHead(head)} className={`flex w-full items-center justify-between gap-3 rounded-[12px] px-3 py-2.5 text-left text-xs ${activeMealHead===head?'bg-[var(--fluent-color-brand-background)] text-[var(--fluent-color-brand-foreground)]':'bg-[var(--fluent-color-neutral-background-1)] text-[var(--fluent-color-neutral-foreground-2)]'}`}><span><span className="font-semibold">{String(index+1).padStart(2,'0')}</span> {label}</span><span className="shrink-0 font-semibold">{count}/5 selected</span></button>})}</nav><div className="space-y-3">{COMMON_FOOD_MEALS.filter(([head])=>head===activeMealHead).map(([head, label]) => {
      ...
      return <div key={head} className="rounded-[18px] border bg-white p-4"><div className="flex flex-wrap justify-between gap-3"><div><h5 className="font-semibold">{label}</h5><p className="mt-1 text-xs text-gray-500">Target: {formatNutrient(generatedMeal?.target?.kcal, 'kcal')} · {formatNutrient(generatedMeal?.target?.protein, 'g protein')}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${selectedCount === 5 ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-800'}`}>{selectedCount}/5 selected · {mealOptions.length} available</span></div>
        <div className="mt-3 space-y-2">{mealOptions.map((option, index) => { const key = option.combinationId || `${head}-${index}`; const selected = selectedIds.has(option.combinationId); const state = persistedIds.has(option.combinationId) ? 'SAVED' : selected ? 'SELECTED' : 'AVAILABLE'; return <article key={key} data-option-type={option.sourceType} data-selection-state={state} className={`rounded-[14px] border p-3 ${selected ? 'border-[var(--fluent-color-brand-stroke-1)] bg-[var(--fluent-color-brand-background-2)]' : 'bg-[var(--fluent-color-neutral-background-2)]'}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2">{!readOnly && ['draft', 'changes_requested'].includes(lifecycle) ? <input type="checkbox" checked={selected} onChange={() => toggleSelection(option)} aria-label={`${selected ? 'Unselect' : 'Select'} ${optionTitle(option)}`} className="h-4 w-4 accent-[var(--fluent-color-brand-background)]" /> : null}<p className="text-sm font-semibold">{optionTitle(option)}</p><span className="rounded-full bg-white px-2 py-1 text-[11px] font-semibold text-gray-600">{state}</span></div><p className="mt-1 text-sm text-gray-600">{optionSummary(option)}</p><div className="mt-2"><NutritionFields nutrition={option.nutrition} /></div>{option.mealQuality ? <p className="mt-2 text-xs text-gray-600">Calories: {option.mealQuality.calories === 'GOOD_FIT' ? 'Good fit' : option.mealQuality.calories} · Protein: {option.mealQuality.protein}</p> : null}</div><button type="button" aria-expanded={Boolean(expanded[key])} aria-label={`View ${optionTitle(option)} details`} onClick={() => setExpanded((current) => ({ ...current, [key]: !current[key] }))} className="rounded-full p-2"><ChevronRight size={17} className={expanded[key] ? 'rotate-90' : ''} /></button></div>{expanded[key] ? <><ComponentTable option={option} readOnly={readOnly || !['draft', 'changes_requested'].includes(lifecycle)} locked={lockedServings[option.combinationId]} onLock={(foodId) => toggleServingLock(option.combinationId, foodId)} onExplore={(mode, component) => openExplorer(mode, option, component, label)} onRemove={(component) => mutate(() => removeFiteatsyCommonFoodComponent(clientId, dietPlanId, option.combinationId, component.foodId, planVersionId))} onServing={(component) => openExplorer('serving', option, component, label)} />{!readOnly && ['draft', 'changes_requested'].includes(lifecycle) ? <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => openExplorer('add', option, null, label)} className="rounded-full border px-3 py-1.5 text-xs font-semibold">Build Meal</button><button type="button" onClick={() => autoBalance(option)} className="rounded-full border px-3 py-1.5 text-xs font-semibold">Auto-Balance</button></div> : null}</> : null}</article>; })}{!mealOptions.length ? <p className="rounded-[12px] border border-dashed p-4 text-sm text-gray-500">{loading ? 'Generating personalised options…' : 'No suitable options are available. Generate alternatives, search governed foods, or build a structured meal.'}</p> : null}</div>
